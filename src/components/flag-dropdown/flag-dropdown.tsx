    ...
    <div className="relative">
      ...
      <div
        className={classNames(
          "absolute -top-10 right-6 flex w-12 flex-col items-center justify-center gap-2 overflow-hidden rounded-md bg-gray-lightest py-2 transition-all delay-100 ease-in-out duration-200 dark:bg-gray-800",
        ...
        {countryFlags.map((flag) => (
          <div key={flag.id} onClick={() => onLanguageChange(flag.language)}>
            <img
              src={flag.icon}
              alt={flag.title}
              className="h-8 w-8 cursor-pointer rounded-full border-2 border-gray-100 hover:border-gray-300 dark:border-gray-600 dark:hover:border-gray-400"
            />
          </div>
        ))}
      </div>
